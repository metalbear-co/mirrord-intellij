...
class BazelBinaryProvider253(
    var env: ExecutionEnvironment,
) : BazelBinaryProvider {
    class BinaryExecutionPlan253(
        private val plan: BinaryExecutionPlan241,
        private val originalEnv: Map<String, String>,
    ) : BinaryExecutionPlan {
        override fun getOriginalEnv(): ImmutableMap<String, String> = originalEnv.toImmutableMap()
        ...
        override fun getBinaryToPatch(): String? = plan.getBinaryToPatch()

        override fun checkExecution(executionInfo: MirrordExecution): String? {
            val originalBinary = ReflectUtils.getPropertyByName(plan.state, "handler.state.blazeBinary.blazeBinary") as String?
            if (SystemInfo.isMac) {
                executionInfo.patchedPath ?: let {
                    ...
                    MirrordLogger.logger.debug(
                        "[${this.javaClass.name}] processStartScheduled: isMac, but not patching SIP (no patched path returned by the CLI).",
                    )
                }
            }
        ...
        override fun restoreConfig(savedConfigData: SavedConfigData) {
            MirrordLogger.logger.debug(
                "[${this.javaClass.name}] restoreConfig: found ${savedConfigData.envVars.size} saved original variables",
            )
            val oldEnvFile = ReflectUtils.getPropertyByName(plan.state, "handler.state.envVars.data.myEnvironmentFile") as String?
            val oldPassParentEnvs = ReflectUtils.getPropertyByName(plan.state, "handler.state.envVars.data.myPassParentEnvs") as Boolean
    ...
    override fun provideTargetBinaryExecPlan(executorId: String): BinaryExecutionPlan {
        ...
        val binaryToPatch =
            if (SystemInfo.isMac) {
                val binaryFromHandler =
                    ReflectUtils.getPropertyByName(state, "handler.state.blazeBinary")?.let { blazeBinaryObject ->
                        val blazeBinaryPath = ReflectUtils.callFunction(blazeBinaryObject, "getBlazeBinary")
                        MirrordLogger.logger.debug(
                            "[${this.javaClass.name}] processStartScheduled: found Bazel binary path in the config: $blazeBinaryPath",
                        )
                        blazeBinaryPath as String?
                    }

                val binaryPath =
                    binaryFromHandler ?: let {
                        val buildSystemProvider =
                            ReflectUtils.callStaticFunction(
                                "com.google.idea.blaze.base.settings.Blaze",
                                "getBuildSystemProvider(Project)",
                                env.project,
                            )!!
    ...
    override fun getBinaryExecPlanClass(): KClass<out BinaryExecutionPlan> = BinaryExecutionPlan253::class
}
