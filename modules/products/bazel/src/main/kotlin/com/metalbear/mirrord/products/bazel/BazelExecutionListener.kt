...
class BazelExecutionListener : ExecutionListener {
    ...
    override fun processStartScheduled(
        ...
        if (!env.runProfile.javaClass.name
                .endsWith("BlazeCommandRunConfiguration")
        ) {
        ...
        val bazelBinaryRunPlan =
            binaryProvider.provideTargetBinaryExecPlan(executorId) ?: run {
            ...
            service.execManager
                ...
                ?.let { executionInfo ->
                    MirrordLogger.logger.debug(
                        "[${this.javaClass.name}] processStartScheduled: adding ${executionInfo.environment.size} environment variables",
                    )
    ...
    override fun processTerminating(
        ...
        this.bazelBinaryExecutionPlans[executorId]
            ?.let {
                val saved =
                ...
                MirrordLogger.logger.debug(
                    "[${this.javaClass.name}] removing bazel binary execution plan for $executorId, execution plan : $bazelBinaryExecutionPlans",
                )
