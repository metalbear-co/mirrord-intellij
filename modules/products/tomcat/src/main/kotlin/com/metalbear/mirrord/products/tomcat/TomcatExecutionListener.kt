...
private fun getTomcatServerPort(): String = System.getenv("MIRRORD_TOMCAT_SERVER_PORT") ?: DEFAULT_TOMCAT_SERVER_PORT
...
data class SavedConfigData(
    var envVars: List<EnvironmentVariable>? = null,
    var scriptInfo: ScriptInfo? = null,
)

data class CommandLineWithArgs(
    val command: String,
    val args: String?,
)
...
class TomcatExecutionListener : ExecutionListener {
        ...
        return if (scriptInfo.USE_DEFAULT) {
            ...
            val commandLine =
                scriptInfo.defaultScript.ifBlank {
                    ...
                    val applicationServer = (env.runProfile as? CommonStrategy)?.applicationServer as? ApplicationServerImpl
                    val persistentData = applicationServer?.persistentData as? TomcatPersistentData
                    val tomcatLocation = persistentData?.HOME
    ...
    override fun processStartScheduled(
            ...
            if (SystemInfo.isMac) {
                ...
                if (startScriptAndArgs == null) {
                    MirrordLogger.logger.warn(
                        "[${this.javaClass.name}] processStartScheduled: could not get script from the configuration.",
                    )
        ...
        if (!alreadyContainsInitTask) {
            config.second.beforeRunTasks = config.second.beforeRunTasks +
                TomcatBeforeRunTaskProvider.TomcatBeforeRunTask {
                    ...
                    service.execManager
                        ...
                        ?.let { executionInfo ->
                            ...
                            MirrordLogger.logger.debug(
                                "[${this.javaClass.name}] processStartScheduled: saving ${originalEnvVars.size} original environment variables",
                            )
                            savedData.envVars = originalEnvVars.toList()

                            MirrordLogger.logger.debug(
                                "[${this.javaClass.name}] processStartScheduled: adding ${mirrordEnv.size} environment variables",
                            )
                            val finalEnvVars =
                                originalEnvVars.associateBy { it.NAME } +
                            ...
                            if (SystemInfo.isMac) {
                                ...
                                executionInfo.patchedPath?.let {
                                    MirrordLogger.logger.debug(
                                        "[${this.javaClass.name}] processStartScheduled: patchedPath is not null: $it, meaning original was SIP",
                                    )
                                    savedData.scriptInfo = startupInfo
                                    if (config.first.startupInfo.USE_DEFAULT) {
                                        MirrordLogger.logger.debug(
                                            "[${this.javaClass.name}] processStartScheduled: using default - handling SIP by replacing config.startupInfo",
                                        )
                                        ...
                                        MirrordLogger.logger.debug(
                                            "[${this.javaClass.name}] processStartScheduled: NOT using default - patching by changing the non-default script",
                                        )
    ...
    private fun restoreConfig(
        executorId: String,
        config: Pair<RunnerSpecificLocalConfigurationBit, RunConfigurationBase<*>>,
    ) {
        MirrordLogger.logger.debug("[${this.javaClass.name}] restoreConfig: $executorId $config")

        val saved =
            savedEnvs.remove(executorId) ?: run {
